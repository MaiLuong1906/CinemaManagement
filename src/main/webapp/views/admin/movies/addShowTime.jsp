                            ...
                            <body>
                                <div>
                                    ...
                                    <% HttpSession sessionObj=request.getSession(false); if (sessionObj !=null) { String
                                        ...
                                        successObj=sessionObj.getAttribute("success"); boolean
                                        success=Boolean.TRUE.equals(successObj) || "true" .equals(successObj); String
                                        messageDb=(String) sessionObj.getAttribute("dbError"); if (message !=null) { %>
                                        <div style="
                                            padding:10px;
                                            margin-bottom:15px;
                                            border-radius:5px;
                                        ...
                                        <% sessionObj.removeAttribute("message"); sessionObj.removeAttribute("success");
                                            ...
                                            <!--input dau vao-->
                                            <form action="<%=request.getContextPath()%>/showtime?action=add"
                                                method="post">
                                                <!--cac hang muc-->
                                                ...
                                                <!--phong chieu-->
                                                <div class="form-group">
                                                    <label for="hallSelect">Chọn phòng chiếu:</label>
                                                    <select id="hallSelect" name="hallId" class="form-control">
                                                        <% List<CinemaHall> hallList = (List<CinemaHall>)
                                                                request.getAttribute("hallList");
                                                                if (hallList != null) {
                                                ...
                                                <div class="form-group">
                                                    <label for="slotSelect">Chọn giờ chiếu:</label>
                                                    <select id="slotSelect" name="timeSlotId" class="form-control">
                                                        <% List<TimeSlot> listChieu = (List<TimeSlot>)
                                                                request.getAttribute("listChieu");
                                                                if (listChieu != null) {
                                                                for (TimeSlot gioChieu : listChieu) {
                                                                %>
                                                                <option value="<%= gioChieu.getSlotId() %>">
                                                                    <%= gioChieu.getSlotId() %> - <%=
                                                                            gioChieu.getSlotName()%> - <%=
                                                                                gioChieu.getStartHour()%>
                                                                </option>
                                                                <% } } %>
                                                    </select>
                                                </div>
