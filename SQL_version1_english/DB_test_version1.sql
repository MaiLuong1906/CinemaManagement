...
--Set loại ghế
INSERT INTO seat_types (type_name, extra_fee) VALUES
                                                  (N'Thường', 0),
                                                  (N'VIP', 20000.00),
                                                  (N'Coupe', 50000.00);
--Phòng chiếu
INSERT INTO cinema_halls (hall_name, total_rows, total_cols, status) VALUES
                                                                         (N'Phòng 1', 5, 8, 1),
                                                                         (N'Phòng 2', 5, 8, 1),
                                                                         (N'Phòng IMAX', 8, 10, 1);
...
DECLARE @hall_id INT, @max_rows INT, @max_cols INT, @hall_name NVARCHAR(50);
DECLARE @r INT, @c INT, @seat_type INT;

-- Duyệt qua tất cả các phòng
DECLARE hall_cursor CURSOR FOR
SELECT hall_id, total_rows, total_cols, hall_name FROM cinema_halls;

OPEN hall_cursor;
FETCH NEXT FROM hall_cursor INTO @hall_id, @max_rows, @max_cols, @hall_name;
...
BEGIN
        ...
        WHILE @c < @max_cols
BEGIN
            -- LOGIC PHÂN LOẠI GHẾ CHO TẤT CẢ CÁC PHÒNG:

            -- 1. Hàng cuối cùng luôn là ghế Coupe (ID = 3)
            IF @r = (@max_rows - 1)
                SET @seat_type = 3;

            -- 2. Các hàng ở giữa (khoảng 1/3 đến 2/3 phòng) là ghế VIP (ID = 2)
            -- Ví dụ: Phòng 5 hàng thì hàng index 2, 3 là VIP. Phòng 8 hàng thì 4, 5, 6 là VIP.
ELSE IF @r >= (@max_rows / 3) AND @r < (@max_rows - 1)
                SET @seat_type = 2;

            -- 3. Các hàng còn lại là ghế Thường (ID = 1)
ELSE
                SET @seat_type = 1;

            -- Thực hiện Insert
INSERT INTO seats (hall_id, seat_code, row_index, column_index, seat_type_id)
VALUES (
           @hall_id,
           CHAR(65 + @r) + CAST((@c + 1) AS NVARCHAR(2)),
           @r, @c, @seat_type
       );
